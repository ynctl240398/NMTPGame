...
#define ASSETS_SECTION_TEXTURES 3
...
void CPlayScene::_ParseSection_SPRITES(string line)
{
	...
	if (tex == NULL)
	{
		//DebugOut((wchar_t)L"[ERROR] Texture ID %d not found!\n", texID);
		return;
	}
...
void CPlayScene::LoadAssets(LPCWSTR assetFile)
{
	...
	while (f.getline(str, MAX_SCENE_LINE))
	{
		...
		case ASSETS_SECTION_TEXTURES: 
			{
				wstring path = ToWSTR(line);
				CTextures::GetInstance()->Load(path.c_str());
				break;
			}
		}
	}
